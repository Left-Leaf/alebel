...
///
/// // 4. 视口居中
/// px = sx + viewportWidth / 2
/// py = sy + viewportHeight / 2
/// ```
///
/// 视口尺寸由 [Camera3DComponent] 在渲染前通过 [setViewportSize] 注入。
/// 未设置时偏移为零，target 映射到视口左上角（退化为无偏移行为）。
class Camera3D extends ChangeNotifier {
  ...
  final Vector2 _viewportCenter = Vector2.zero();
  ...
  /// 视口中心偏移（只读）。在矩阵中作为平移分量叠加，
  /// 使 [target] 映射到视口中心而非左上角。
  Vector2 get viewportCenter => _viewportCenter;

  /// 设置视口尺寸，内部存储其半尺寸作为居中偏移。
  ///
  /// 值未变化时不触发重算。由 [Camera3DComponent] 在渲染前调用。
  void setViewportSize(double width, double height) {
    final halfW = width / 2;
    final halfH = height / 2;
    if (_viewportCenter.x == halfW && _viewportCenter.y == halfH) return;
    _viewportCenter.setValues(halfW, halfH);
    _markAsModified();
  }
  ...
  ///   tx  = viewportCenter.x - (m00 * target.x + m01 * target.y)
  ///   ty  = viewportCenter.y - (m10 * target.x + m11 * target.y)
  /// ```
  ...
  ///
  /// 矩阵的平移分量包含视口居中偏移，提取 target 前先减去 [_viewportCenter]。
  ///
  /// 推导公式：
  /// ```
  ///   zoom  = √(m00² + m01²)
  ...
  void setFromMatrix(Float64List matrix) {
    ...
    final m01 = matrix[4];
    final m11 = matrix[5];
    final tx = matrix[12] - _viewportCenter.x;
    final ty = matrix[13] - _viewportCenter.y;
  ...
  void _recomputeMatrix() {
    ...
    m[10] = 1;
    m[11] = 0;
    m[12] = _viewportCenter.x - (m00 * _target.x + m01 * _target.y);
    m[13] = _viewportCenter.y - (m10 * _target.x + m11 * _target.y);
    m[14] = 0;
    m[15] = 1;
  }
}
